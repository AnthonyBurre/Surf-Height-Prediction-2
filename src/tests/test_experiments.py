...
def _synthetic_df(n: int = 120) -> pd.DataFrame:
    ...
    df = pd.DataFrame(
        {
            "hsig_m": 1.2 + 0.5 * np.sin(np.arange(n) / 24) + 0.05 * rng.standard_normal(n),
            "hmax_m": 2.0 + 0.05 * rng.standard_normal(n),
        },
        index=idx,
    )
...
def test_model_hyperparams_uses_get_params_for_sklearn():
    params = _model_hyperparams(Ridge(alpha=2.5, fit_intercept=False))
    assert params["alpha"] == 2.5
    assert params["fit_intercept"] is False


def test_model_hyperparams_uses_vars_for_custom_forecaster():
    params = _model_hyperparams(PersistenceForecaster(target_col="custom"))
    assert params == {"target_col": "custom"}


def test_model_hyperparams_handles_none_model():
    assert _model_hyperparams(None) == {}
...
def test_log_run_appends_one_record(tmp_path, split):
    ...
    assert parsed == record  # exact round-trip
